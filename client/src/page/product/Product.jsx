...
console.log(categoryFilter,"dsaf")
  const categoryMap = [
  ...
  return (
    <div>
      ...
      <div>
        ...
        <select id="category" className="text-black" value={selectCategory} onChange={(e)=>setSelectCategory(e.target.value)}>
          <option value="">All Categories</option>

          {
            categoryMap?.map(opt=><option key={Math.random()} value={opt}>{opt.charAt(0).toUpperCase() + opt.slice(1)}</option>)
          }
        </select>
      </div>
